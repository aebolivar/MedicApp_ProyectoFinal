...
<head>
	...
	<body>
		...
		<?php
			...
			$conexion2=$base->query("SELECT * FROM productos_cliente");
	
			$registros2=$conexion2->fetchAll(PDO::FETCH_OBJ);	
				
			if(isset($_POST["anadir_carrito"])){
				$nombre=$_POST["nom"];
				$precio=$_POST["pre"];
				$direccion=$_POST["dir"];

				$sql="INSERT INTO productos_cliente (Nombre, Precio, Direccion) VALUES (:nom, :pre, :dir)";
				...
				?>
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
			 		<section id="producto">
						...
						<input style="text-align: center;width: 80%; margin: auto;" type="text" name="pre" value="<?php echo $persona->Precio ?>" readonly>
						<input style="text-align: center;width: 80%; margin: auto;" type="submit" name="anadir_carrito" value="Añadir A Carrito">
						<input style="visibility: hidden;" type="text" name="dir" value="<?php echo $persona->Direccion ?>" readonly>
